...
const phrases = [
    "No", "Ay", "Isa!", "Pweaseee",
    "Last chance!", "babe naman!", "Pls?", "ayaw mo talaga?",
    "final na yan??", "Error 404: No not found"
];
let phraseIndex = 0;
...
const moveNoButton = () => {
    ...
    // Generate coordinates relative to the top-left (0,0)
    const randomX = Math.random() * availableWidth + padding;
    const randomY = Math.random() * availableHeight + padding;

    // Apply fixed positioning to ensure it's not trapped by flex containers
    noBtn.style.position = 'fixed';
    noBtn.style.left = '0';
    noBtn.style.top = '0';
    noBtn.style.margin = '0'; // Clear any CSS margins
